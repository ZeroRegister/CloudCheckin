#!/usr/bin/env python3
"""
NodeSeek 自动签到脚本 - 青龙面板适配版
环境变量: NODESEEK_COOKIE 或 NS_COOKIE (多账号用 & 分隔)
"""
import sys
import os
...
# Get COOKIE from environment variable (support both names)
cookies = os.environ.get('NODESEEK_COOKIE', '').strip()
if not cookies:
    cookies = os.environ.get('NS_COOKIE', '').strip()

if not cookies:
    print("❌ 错误: 环境变量 NODESEEK_COOKIE 或 NS_COOKIE 未设置")
    sys.exit(1)
...
print(f"🚀 NodeSeek 签到开始，共 {len(cookie_list)} 个账号")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get('success') or result.get('message', '').find('已签到') >= 0:
                print(f"✅ NodeSeek 账号 {idx+1} 签到成功!")
            else:
                print(f"✅ NodeSeek 账号 {idx+1} 请求成功: {response.text}")
        else:
            print(f"❌ NodeSeek 账号 {idx+1} 签到失败: {response.text}")
            sys.exit(1)
    
    except Exception as e:
        print(f"❌ NodeSeek 账号 {idx+1} 签到出错: {e}")
        sys.exit(1)
